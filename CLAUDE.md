...
/log/atlas/        — QSO atlas: year-from-the-book map and season from the published QRZ log
...
/log/atlas/index.html    — QSO atlas: hops from the book plus a written season
...
hand-written HTML files that link it. Blog posts are the exception: structured
JSON source is committed under `/content/blog/`, then the matching builder
regenerates static HTML so publishing does not require HTML surgery.
